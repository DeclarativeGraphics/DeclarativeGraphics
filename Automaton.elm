...
splitEvents = Step <| \ input -> case input of
...
----- ELM INTERFACE -----
run : Automaton (Event i) (Behavior o) -> Signal i -> Signal o
run automaton sig =
  let (aut,init) = step automaton NoEvent
      stepInput input (automaton,state) = step automaton (Event input)
  in lift (extractValue . snd) <| foldp stepInput (aut,init) sig
